...
public class OrderDataController {
    ...
    public ResponseEntity<ApiResponse> getOrderById(@PathVariable(name = "id") Long id) {
        ...
        optionalOrderData.orElseThrow(() -> new ResourceNotFoundException("Oder-ul cu id-ul " + id + " nu a fost gasit"));
    ...
    public ResponseEntity<ApiResponse> updateOrder(@RequestBody OrderData orderData) {
        OrderData updatedOrder = this.orderDataService.updateOrder(orderData);

        return ResponseEntity.ok(ApiResponse.success("Order-ul a fost actualizat cu success", updatedOrder));
    }
